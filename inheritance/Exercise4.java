...
class RegionalBank extends Bank {
    ...
    public RegionalBank(String name, int customers, double[][] accounts) {
        super.setName(name);
        super.setCustomers(customers);
        this.regionalAccounts = accounts;
    }
    ...
    public double regionalTotal() {
        double total = 0;
        for (double[] account : regionalAccounts) {
            for (double subAccount : account) {
                total += subAccount;
            }
        }
